...
const retorno = arrays.reduce((value, final) => {
...
function getDados(array) {
  ...
  array.forEach((element) => {
    dadosMother = getMother(array, element.mother);
    output.push({...element, dadosMother});
  });
  output = output.filter((element) => {
    return element.dadosMother != undefined;
  });
  return avarege(output);
}
function getMother(array, mother) {
  var dadosMother = array.filter((m) => {
    return m.name === mother;
  });
  return dadosMother[0];
}
var avarege = (array) => {
  var ageMothers = [];
  array.forEach((person) => {
    ageMothers.push(person.born - person.dadosMother.born);
  });
  totalAgeMothers = ageMothers.reduce((start, next) => {
    return start + next;
  }, 0);
  return parseInt(totalAgeMothers / ageMothers.length);
};
// console.log(getDados(ancestry));
...
function average(array) {
  ...
  ancestry.forEach(person=>{
    var centu = Math.ceil(person.died/100)
    switch (centu) {
      case 16:
        century[0].push(person.died - person.born)
        break;
      case 17:
        century[1].push(person.died - person.born)
        break;
      case 18:
        century[2].push(person.died - person.born)
        break;
      case 19:
        century[3].push(person.died - person.born)
        break;
      case 20:
        century[4].push(person.died - person.born)
        break;
      case 21:
        century[5].push(person.died - person.born)
        break;
    }
  })
var  cen = 16
var output = {}
century.forEach(element=>{
  output[cen] = average(element)
  cen++
})
console.log(output);
